# Design System: Tailwind CSS - Rapidly build modern websites without ever leaving your HTML.

> Extracted from [https://tailwindcss.com](https://tailwindcss.com) using `npx brandmd`.
>
> Drop this file in your project root so Claude Code, Cursor, Gemini CLI, Codex, or Google Stitch can match Tailwind CSS - Rapidly build modern websites without ever leaving your HTML.'s brand when generating UI.
>
> Generate your own: `npx brandmd https://yoursite.com` ([repo](https://github.com/yuvrajangadsingh/brandmd))
## 1. Visual Theme & Atmosphere

**Overall mood:** Dark and moody

**Density:** Moderate. The layout uses a varied spacing scale.

**Shape language:** Rounded, friendly aesthetic with generous corner radii.

**Depth:** Uses 5 shadow styles for layering and elevation.

## 2. Color Palette & Roles

- **White** (`#FFFFFF`) — Page background
- **Dark Blue** (`#0307120D`) — Dark background / footer
- **Blue** (`#00BCFF1A`) — Accent background
- **Dark Purple** (`#5D0EC0`) — Dark background / footer
- **Dark Orange** (`#7E2A0C`) — Dark background / footer
- **Pink** (`#F6339A`) — Accent background
- **Black** (`#000000`) — Primary text
- **Muted Blue** (`#90A1B9`) — Muted text
- **Blue** (`#00A6F4`) — Link / accent text
- **Black** (`#0000000D`) — Divider / border

## 3. Typography Rules

**Primary font:** inter
**Secondary font:** plexMono

**Fonts by role:**
- Headings: inter
- Body: inter

**Type scale:**
- Headings: 24px, 30px, 36px, 40px, 96px
- Body / UI: 14px, 16px, 17px, 18px, 20px
- Captions / Small: 12px, 13px

**Weights in use:** 400, 500, 600, 700

**Line heights:** 24px, 28px, 20px, 40px, 16px, 48px, 96px, 36px

**Letter spacing:** 1.2px, -2px, 1.4px, -4.8px

## 4. Component Stylings

### Buttons
- Text color: `oklab(0.129999 -0.00404751 -0.027702 / 0.5)`
- Corner radius: 3.35544e+07px
- Padding: 8px 16px 8px 16px
- Font: 14px, weight 400

### Cards
- Shadow: `rgba(0, 0, 0, 0) 0px 0px 0px 0px, oklab(0.129999 -0.00404751 -0.027702 / 0.1) 0px 0px 0px 1px inset, rgba(0, 0, 0, 0) 0px 0px 0px 0px, rgba(0, 0, 0, 0) 0px 0px 0px 0px, rgba(0, 0, 0, 0) 0px 0px 0px 0px`
- Corner radius: 4px

### Inputs
- Border: 0px solid rgb(0, 0, 0)
- Padding: 0px 0px 0px 0px
- Font size: 16px

## 5. Layout Principles

**Spacing scale:** 4px, 8px, 12px, 16px, 24px, 32px, 36px, 40px

**Base unit:** 4px grid (values are mostly multiples of 4).

**Border radii:** 4px, 8px, 12px, 16px, 16px 16px 0px 0px, 24px, 32px, 3.35544e+07px

## 6. Guidelines

### Do
- Use 4px grid for all spacing
- Use `#00BCFF1A` for primary actions and CTAs
- Stick to 4 font weights: 400, 500, 600, 700
- Use `inter` as the primary typeface

### Don't
- Don't introduce colors outside the palette above
- Don't mix fonts beyond inter and plexMono
- Don't use inline styles when the design system covers the pattern
- Don't use border-radius values outside: 4px, 8px, 12px, 16px, 16px 16px 0px 0px, 24px, 32px, 3.35544e+07px

---

*This DESIGN.md was generated by [brandmd](https://github.com/yuvrajangadsingh/brandmd). Drop it into your project root and AI coding agents (Claude Code, Cursor, Gemini CLI) will use it to generate on-brand UI.*